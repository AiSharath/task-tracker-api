    ...
    return (
        <>
            <div className="tasklist-container">
                <div className="tasklist-header">  {/* ← Bug 6: was "classname" not "className" */}
                    <h2>Available Tasks</h2>
                    <button onClick={() => navigate("/tasks/create")}>Add Tasks</button>
                </div>
                <div className="tasks-grid">
                    {tasks.map(task => (
                        <div key={task._id} className={`task-card ${task.status === "assigned" ? "assigned" : ""}`}>  {/* ← Bug 7: & should be $ */}
                            <div className="task-top">
                                <h4>{task.title}</h4>
                                <span className={`status ${task.status}`}>{task.status}</span>  {/* ← same & → $ fix */}
                            </div>
                            <p>{task.description}</p>
                            <div className="task-bottom">
                                <span className="due-date">{task.dueDate?new Date(task.dueDate).toLocaleDateString():"No due data"}</span>
                                {task.status === "pending" && (  // ← Bug 8: was tasks.status not task.status
